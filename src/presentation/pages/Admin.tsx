  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <header className="flex flex-col md:flex-row items-center justify-between px-4 md:px-10 py-4 bg-white shadow-md">
       ...
       {/* Filter Button */}
       <div className={`flex ${isListView ? "justify-between" : "justify-end"} px-4 md:px-10 py-4`}>
  ...
  <button
    onClick={() => {
      if (selectedInput[category] && selectedMonth[category]) {
        ...
        setSelectedMonth((prev) => ({ ...prev, [category]: "" })); 
      }
    }}
  >
    <img src={CheckCircleSuccess} alt="Checkmark" className="w-7 h-7 cursor-pointer" />
              ...
              const filtered = developers.filter((dev) => {
                return Object.entries(selectedItems).every(([category, items]) => {
                  return items.some((item) => {
                    ...
                    return dev.role.toLowerCase().includes(item.toLowerCase());
                  });
                });
              });
      ...
      <div className={`px-4 md:px-10 ${isListView ? "flex flex-col gap-4" : "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6"}`}>
    ...
    <div key={index} ref={index === visibleDevelopers.length - 1 ? lastElementRef : null}>
      {isListView ? (
        ...
        <div className="flex items-center bg-white shadow-md rounded-lg p-4">
          <img src={dev.image} alt={dev.name} className="w-16 h-16 rounded-full object-cover mr-4" />
          <div>
