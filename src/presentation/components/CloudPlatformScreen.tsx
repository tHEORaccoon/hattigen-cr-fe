...
const CloudPlatformScreen = () => {
      const [formData, setFormData] = useState<CloudPlatformScreenProps>({
        platform: "",
        experience: 1,
      });
        ...
        const handleDeleteCloud = (index: number) => {
            setCloudPlatforms(cloudPlatforms.filter((_, i) => i !== index));
        };
        ...
        <div className="flex flex-col md:w-1/2">
          <div className="grid grid-rows-3 md:flex md:gap-20 md:justify-between">
            <div className="flex flex-col w-full md:w-[500px]">
              <label htmlFor="platform" className="mb-2">Language</label>
              <input
                type="text"
                name="platform"
                placeholder="Find a cloud platform"
          ...
          <div className="flex flex-wrap gap-2 mt-5">
              {cloudPlatforms.map((cloud, index) => (
                <div
                  key={index}
                ...
                >
                  ...
                  <span className="text-sm text-gray-600">{cloud.experience}</span>

                  {isEditing && (
                    <button
                      onClick={() => handleDeleteCloud(index)}
...
export default CloudPlatformScreen
